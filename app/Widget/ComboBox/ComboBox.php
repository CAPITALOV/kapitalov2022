 ...
 * Класс ComboBox
 *
$field->widget('cs\Widget\ComboBox\ComboBox', [
...
class ComboBox extends InputWidget
{
    ...
    public function run()
    {
        ...
        return Yii::$app->view->renderFile('@csRoot/Widget/ComboBox/template.php', [
            'items'      => $this->items,
...
$('#{$this->attrId}').combobox({$options});
JS
        );
        if ($this->events) {
            foreach($this->events as $name => $function) {
                $this->getView()->registerJs(<<<JS
$('#{$this->attrId}').on('{$name}', {$function});
JS
                );
            }
        }
